...
def plot_stocks(filename, stock_names):
    ...
    stock_weekly = np.log(stock_weekly + 1)
    ...
    stock_monthly = np.log(stock_monthly + 1)
    ...
    stock_daily = np.log(stock_daily + 1)
    ...
    for i in range(20):
        ...
        axs[0].hist(stock_daily.iloc[:, i],
                    label="Daily", bins=40, density=True)
        ...
        axs[1].hist(stock_weekly.iloc[:, i],
                    label="Weekly", bins=30, density=True)
        ...
        axs[2].hist(stock_monthly.iloc[:, i],
                    label="Monthly", bins=20, density=True)
        ...
        plt.suptitle(
            f'Normalized log return of {stock_names[stock_monthly.columns[i]]} stock.')
        plt.tight_layout()
        plt.savefig(
            f'Q3stockplots/{stock_monthly.columns[i].replace(".", "_")}_logreturns.png')
        # plt.show()
        plt.close()
...
def plot_indices(indices):
    for index in indices:
        ...
        index_weekly = np.log(index_weekly + 1)
        ...
        index_monthly = np.log(index_monthly + 1)
        ...
        index_daily = np.log(index_daily + 1)
        ...
        plt.suptitle(f'Normalized log return of {index}')
        plt.tight_layout()
        plt.savefig(f'Q3indexplots/{index.replace(" ", "_")}_logreturns.png')
        # plt.show()
        plt.close()
...
def main():
    ...
    folder_name = 'Q3indexplots'
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)

    folder_name2 = 'Q3stockplots'
