...
def get_historical_volatility(stocks_type, time_period):
  ...
  if stocks_type == 'BSE':
    stocks_name = ['SUNPHARMA.BO', 'TITAN.BO', 'WIPRO.BO', 'SBIN.BO', 'ULTRACEMCO.BO', 'RELIANCE.BO', 'MARUTI.BO', 'LT.BO', 'NTPC.BO', 'HINDUNILVR.BO',
                   'ZOMATO.BO', 'ZEEL.BO', 'TVSMOTOR.BO', 'PIDILITIND.BO', 'NAUKRI.BO', 'IRCTC.BO', 'DLF.BO', 'BANKBARODA.BO', 'MFSL.BO', 'COLPAL.BO', '^BSESN']
    filename = './bsedata1.csv'
  else:
    stocks_name = ['ADANIENT.NS', 'ASIANPAINT.NS', 'AXISBANK.NS', 'BAJAJ-AUTO.NS', 'BPCL.NS', 'BHARTIARTL.NS', 'BRITANNIA.NS', 'CIPLA.NS', 'HCLTECH.NS',
                   'TATASTEEL.NS', 'PEL.NS', 'JUBLFOOD.NS', 'UBL.NS', 'MRF.NS', 'BERGEPAINT.NS', 'SIEMENS.NS', 'HINDPETRO.NS', 'HAVELLS.NS', 'INDIGO.NS', 'DABUR.NS', '^NSEI']
...
def price_computation(stocks_type):
  ...
  for idx1 in range(len(stocks_name)):
    ...
    for i in range(len(A)):
      ...
      if i == 5:
        plt.savefig('./' + stocks_type + '/call/' +
                    stocks_name[idx1] + '_Call_1.png')
        plt.clf()
        # plt.show()
    plt.savefig('./' + stocks_type + '/call/' +
                stocks_name[idx1] + '_Call_2.png')
    plt.clf()
    # plt.show()
    ...
    for i in range(len(A)):
      ...
      if i == 5:
        plt.savefig('./' + stocks_type + '/put/' +
                    stocks_name[idx1] + '_Put_1.png')
        plt.clf()
        # plt.show()
    plt.savefig('./' + stocks_type + '/put/' + stocks_name[idx1] + '_Put_2.png')
    plt.clf()
    # plt.show()
    ...
    plt.title("Historical Volatility vs time period for {}".format(
        stocks_name[idx1]))
    plt.savefig('./' + stocks_type + '/vol/' +
                stocks_name[idx1] + '_volatility.png')
    plt.clf()
    # plt.show()
